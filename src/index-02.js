...
//   measurements: types.optional(types.array(Measurement), [])
// });
    ...
    if (keys.length === 0) {
      ...
      for (const key in incomingData) {
        self.datapoints.keys().indexOf(key) > -1
          ? self.datapoints.get(key).push(Measurement.create(incomingData[key]))
          : self.datapoints.set(key, [Measurement.create(incomingData[key])]);
      }
